...
function drawBlackout(cx, cy) {
    if (!startTime) {
        ...
        blackoutProgress = Math.min(1, (new Date() - startTime - BLACKOUT_TIME) / 500);
    }
    ...
    let rx, ry;
    let pf, px, py;
    let radius = Math.sqrt(canvasWidth * canvasWidth + canvasHeight * canvasHeight) * blackoutProgress;
